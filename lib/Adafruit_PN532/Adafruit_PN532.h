// LOCAL FORK (Santa Muerte badge): vendored from adafruit/Adafruit PN532
// 1.3.4 with one addition -- lastSAK()/lastATQA() expose the SEL_RES/SENS_RES
// bytes that readDetectedPassiveTargetID() already parses but the upstream
// library discards, so the firmware can name a card's type even when it cannot
// read its contents. Search "SM-FORK" for the three touch points. Do not pull
// upstream over this without re-applying them.
/**************************************************************************/
/*!
...
public:
  ...
  // SM-FORK: identity bytes from the most recent passive activation. SAK
  // (SEL_RES) names the card family -- 0x08 Classic 1K, 0x18 Classic 4K, the
  // 0x20 bit ISO14443-4 (DESFire/Type 4), 0x00 Ultralight/NTAG. ATQA (SENS_RES)
  // is the 2-byte answer-to-request. Valid after a successful
  // readPassiveTargetID()/readDetectedPassiveTargetID().
  uint8_t lastSAK(void) const { return _lastSAK; }
  uint16_t lastATQA(void) const { return _lastATQA; }
...
private:
  ...
  uint8_t _lastSAK = 0;   // SM-FORK
  uint16_t _lastATQA = 0; // SM-FORK
